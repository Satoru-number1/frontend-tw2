...
import { Search, Plus, Trash2, Edit3, X, UserPlus, Mail, Lock, Shield, Phone, MapPin, User } from 'lucide-react';
...
export default function UsersView({ triggerToast }: UsersViewProps) {
  ...
  const getRoleEnum = (roleStr: string): number => {
    ...
    if (roleStr === 'Encargado de Almacén' || roleStr === 'EncargadoAlmacen') return 1;
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const mapped = data.map((u: any, idx: number) => ({
        ...
        status: u.estado === 0 || u.Estado === 0 || u.estado === 'Activo' ? 'Activo' : 'Inactivo'
      }));
      setUsersList(mapped);
    } catch (err) {
  ...
  const handleRegisterUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      triggerToast(err.response?.data?.mensaje || 'Error al registrar nuevo usuario.');
    } finally {
  ...
  const handleEditRole = async (e: React.FormEvent) => {
    ...
    try {
      ...
      triggerToast(err.response?.data?.mensaje || 'Error al actualizar rol del usuario.');
    } finally {
      setIsSubmitLoading(false);
    }
  };
        ...
        <div className="overflow-x-auto w-full">
          <table className="w-full text-left border-collapse text-[13px]">
            ...
            <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
              {isLoading ? (
                <tr>
                  <td colSpan={5} className="py-10 text-center text-neutral-400 dark:text-neutral-500">
                    Cargando usuarios...
                  </td>
                </tr>
              ) : usersList.length === 0 ? (
                ...
                usersList
                  .filter(u => {
                    ...
                    const matchesRole = userFilterRole === 'Todos' || u.role === userFilterRole;
                    return matchesSearch && matchesRole;
                  })
                  .map(user => (
                    <tr key={user.id} className="hover:bg-neutral-50/40 dark:hover:bg-neutral-950/20 transition-colors">
                      <td className="py-3.5 px-5 font-bold text-neutral-900 dark:text-white">{user.name}</td>
                      <td className="py-3.5 px-5 text-neutral-550 dark:text-neutral-400 font-mono">{user.email}</td>
          ...
          <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 shadow-xl max-w-xl w-full p-6 flex flex-col gap-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleRegisterUser} className="flex flex-col gap-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] font-bold text-neutral-500 uppercase tracking-wider mb-1 block">Nombre *</label>
                  <input
                    type="text"
                    required
                    value={newNombre}
                ...
                <div>
                  <label className="text-[11px] font-bold text-neutral-500 uppercase tracking-wider mb-1 block">Apellido *</label>
                  <input
                    type="text"
                    required
                    value={newApellido}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] font-bold text-neutral-500 uppercase tracking-wider mb-1 block">CI *</label>
                  <input
                    type="text"
                    required
                    value={newCi}
                ...
                <div>
                  <label className="text-[11px] font-bold text-neutral-500 uppercase tracking-wider mb-1 block">Teléfono</label>
                  <input
                    type="text"
                    value={newTelefono}
              ...
              <div>
                <label className="text-[11px] font-bold text-neutral-500 uppercase tracking-wider mb-1 block">Dirección</label>
                <input
                  type="text"
                  value={newDireccion}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] font-bold text-neutral-500 uppercase tracking-wider mb-1 block">Email *</label>
                  <input
                    type="email"
                    required
                    value={newEmail}
                ...
                <div>
                  <label className="text-[11px] font-bold text-neutral-500 uppercase tracking-wider mb-1 block">Contraseña *</label>
                  <input
                    type="password"
                    required
                    value={newPassword}
              ...
              <div>
                <label className="text-[11px] font-bold text-neutral-500 uppercase tracking-wider mb-1 block">Asignar Rol</label>
                <select
                  value={newRole}
            ...
            <form onSubmit={handleEditRole} className="flex flex-col gap-4">
              <div>
                <p className="text-[13px] text-neutral-600 dark:text-neutral-450 leading-relaxed">
                  Modifique el rol para el usuario: <strong className="text-neutral-900 dark:text-white">{selectedUser.name}</strong> ({selectedUser.email}).
                </p>
              </div>

              <div>
                <label className="text-[11px] font-bold text-neutral-500 uppercase tracking-wider mb-1 block">Nuevo Rol</label>
                <select
                  value={editRole}
