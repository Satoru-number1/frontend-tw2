...
import { Search, FileText, Trash2, Plus, TrendingUp, ShoppingBag, CreditCard, QrCode, DollarSign, CheckCircle2, Download } from 'lucide-react';
...
export default function SalesView({ triggerToast }: SalesViewProps) {
  ...
  const addProductToTicket = (prod: Product) => {
    ...
    const existing = ticketItems.find(item => item.codigoBarras === prod.codigoBarras);
    if (existing) {
      if (existing.qty >= prod.stock) {
        triggerToast(`No puede vender más de ${prod.stock} unidades de este producto.`);
        return;
      }
      setTicketItems(ticketItems.map(item => item.codigoBarras === prod.codigoBarras ? { ...item, qty: item.qty + 1 } : item));
    } else {
  ...
  const removeItemFromTicket = (codigoBarras: string) => {
    setTicketItems(ticketItems.filter(item => item.codigoBarras !== codigoBarras));
  };
    ...
    setTicketItems(ticketItems.map(item => {
      if (item.codigoBarras === codigoBarras) {
        ...
        if (nextQty > item.maxStock) {
          triggerToast(`Stock insuficiente. Solo quedan ${item.maxStock} unidades.`);
          return item;
        }
  ...
  const handleCheckout = async () => {
    ...
    if (paymentMethod === 'Efectivo' && parsedMonto < totalTicket) {
      triggerToast('El monto en efectivo recibido es inferior al total a pagar.');
      return;
    }
    ...
    try {
      ...
      if (ventaData) {
        try {
          generarComprobantePdf({
            ...
            fechaCompra: ventaData.fechaCompra || ventaData.FechaCompra || new Date().toISOString(),
      ...
      triggerToast(err.response?.data?.mensaje || 'Error al procesar la venta en el servidor.');
    } finally {
      setIsSubmitLoading(false);
    }
  };

  const handleDownloadInvoice = (ventaId: number) => {
    triggerToast('Esta funcionalidad ahora genera el comprobante automáticamente al completar la venta.');
  };
      ...
      <div className="lg:col-span-2 flex flex-col gap-5">
        ...
        <div className="bg-white dark:bg-neutral-900 border border-neutral-200/80 dark:border-neutral-800/80 rounded-2xl p-5 shadow-sm">
          <div className="flex justify-between items-center mb-4 select-none">
            ...
            <span className="text-[10px] text-neutral-400 dark:text-neutral-500 font-bold uppercase tracking-wider">Pulse para agregar</span>
          </div>
            ...
            {isLoading ? (
              ...
              filteredProducts.map(prod => (
                ...
                >
                  ...
                  <p className="text-[12px] font-bold text-neutral-900 dark:text-white group-hover:text-black dark:group-hover:text-white transition-colors truncate w-full">{prod.nombre}</p>
                  <p className="text-[14px] font-black text-neutral-805 dark:text-neutral-200 mt-0.5">Bs {prod.precio.toFixed(2)}</p>
                  <div className="flex justify-between items-center w-full mt-1.5">
                    <span className="text-[10px] text-neutral-400 font-mono">Stock: {prod.stock}</span>
                    {prod.stock <= prod.stockMinimo && prod.stock > 0 && (
                      <span className="px-1.5 py-0.5 rounded text-[8px] font-black bg-amber-500/10 text-amber-500 border border-amber-550/10 uppercase">Crítico</span>
                    )}
                  </div>
                </button>
              ))
      ...
      <div className="flex flex-col gap-6">
        {saleResult && (
          <div className="bg-white dark:bg-neutral-900 border border-neutral-200/80 dark:border-neutral-800/80 rounded-2xl p-5 shadow-sm flex flex-col gap-4 animate-fadeIn">
            <div className="flex items-center gap-2 border-b border-neutral-100 dark:border-neutral-800 pb-3">
              ...
              <div>
                <h4 className="text-[13px] font-bold text-neutral-900 dark:text-white">¡Venta Exitosa!</h4>
                <p className="text-[11px] text-neutral-500">Transacción #{saleResult.venta?.ventaId || '0000'}</p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2 text-[13px]">
              <div className="flex justify-between">
                ...
                <span className="font-bold">Bs {(saleResult.venta?.total || 0).toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-neutral-500">Método de Pago</span>
                <span className="font-bold">{saleResult.metodoPago?.nombre || 'Efectivo'}</span>
              </div>
              {saleResult.vuelto > 0 && (
        ...
        <div className="bg-white dark:bg-neutral-900 border border-neutral-200/80 dark:border-neutral-800/80 rounded-2xl p-5 shadow-sm flex flex-col gap-5">
          <div className="flex justify-between items-center select-none border-b border-neutral-100 dark:border-neutral-800 pb-3">
            ...
            <span className="text-[11px] font-bold text-neutral-400 tracking-wider">POS ACTIVO</span>
          </div>
          ...
          <div className="flex flex-col gap-3 max-h-56 overflow-y-auto pr-1">
            {ticketItems.length === 0 ? (
              ...
              ticketItems.map(item => (
                <div key={item.codigoBarras} className="flex justify-between items-start gap-2 border-b border-neutral-50 dark:border-neutral-950 pb-2">
                  <div className="overflow-hidden flex-1">
                    ...
                    <div className="flex items-center gap-1.5 mt-1 select-none">
                      ...
                      <span className="text-[10px] text-neutral-400 dark:text-neutral-500 pl-1">x Bs {item.price.toFixed(2)}</span>
                    </div>
                  </div>
          ...
          <div className="border-t border-neutral-100 dark:border-neutral-800 pt-3 flex flex-col gap-2 text-[13px] font-medium text-neutral-500 dark:text-neutral-400 select-none">
            <div className="flex justify-between">
              <span>Subtotal</span>
              <span className="text-neutral-800 dark:text-neutral-200">Bs {subtotalTicket.toFixed(2)}</span>
            </div>
            <div className="flex justify-between">
              <span>Descuento</span>
              <span className="text-neutral-800 dark:text-neutral-200">Bs {discountTicket.toFixed(2)}</span>
            </div>
          ...
          {paymentMethod === 'Efectivo' && (
            <div className="grid grid-cols-2 gap-4 select-none">
              <div>
                <label className="text-[11px] font-bold text-neutral-450 dark:text-neutral-500 uppercase tracking-widest block mb-1">Monto recibido</label>
                <input
                  type="text"
                  value={montoRecibido}
              ...
              <div>
                <span className="text-[11px] font-bold text-neutral-450 dark:text-neutral-500 uppercase tracking-widest block mb-1">Cambio (vuelto)</span>
          ...
          >
            {isSubmitLoading ? 'Procesando Venta...' : 'Confirmar Venta y Cobrar'}
          </button>
        </div>
      </div>
    </div>
